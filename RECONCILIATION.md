...
## Open observations (reported, not decided)

1. **Mass template self detection.** `parseMassRows` marks a row as self only when the evaluator email equals the participant email, as before. Increment 13's `isSelfRole` is not applied there, because the contract says the mass template omits the self row and the importer synthesises one. A mass-template row labelled `Autoevaluare` with a different email is therefore mapped to a `Manager` relation, not to self. Whether that is right is a product question.
...
|---|---|---|
| `app.js` | `535e3fad361b87d49482d906a22f97059fd638e05809f26eefbeecb8a9af0614` | `b0d6f2f95015c9c9cff39514cf5005e34270df464da428331e85927d1c22d4c7` |
...
| `index.html` | `9a4c950aaefcd205e652fb7d700f75a2281d4324d8e9f6ebdc5e80cbec7038ff` | `17587aefb961b9dbb10ab7ff4b855821b089f7e14274f18b5551f53974c78b8f` |
...
|---|---|---|---|
| `src/` | Canonical source of the served app (ES-module form), same assets | None. The placeholder `ex. TLJ 2026` is already live since `7599b4d` | Publish |
...
| `docs/platform-bridge-proposal.md` | Proposal for respondent enrollment in the future platform | None personal. **Flag:** internal product-direction note ("proposal only", open decisions for Vlad/Architect) | Publish per DoD 1 (docs). **Vlad/Inspector to confirm** it may be public; otherwise move it to `bhb-platform` `10-project/standalone-history/` |
