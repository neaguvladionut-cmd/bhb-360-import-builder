...
Object.assign(text.ro, {
  allocationHelp: "Opțional, doar pentru un proiect deja activ",
  privacyLine: "Nicio încărcare online, fără analize de utilizare și fără salvare în browser. Resetarea șterge datele din memorie; fișierele deja descărcate rămân pe dispozitiv.",
  reviewLead: "Erorile sunt izolate pentru fiecare participant și sursă. Fișierul final devine disponibil numai după rezolvarea tuturor blocajelor.",
  name: "Numele participantului",
  email: "Adresa de email a participantului",
  conflict: "Aceeași adresă de email apare cu nume diferite",
  recommended: "Recomandat pe baza exportului",
  recoveryText: "Corectează fișierul indicat, apoi folosește opțiunea Înlocuiește pentru sursa lui sau elimină-l și încarcă-l din nou. Revizuiește și regenerează ieșirile. Dacă închizi sau resetezi pagina, memoria se golește și sursele trebuie reîncărcate.",
  sourceRejectedIssue: "Sursa nu a putut fi citită. Corectează fișierul, apoi elimină sursa sau înlocuiește-o; nu reutilizăm în tăcere o stare mai veche.",
});
Object.assign(text.en, {
  allocationHelp: "Optional, only for an already active project",
  privacyLine: "No online uploads, analytics, or browser storage. Reset clears in-memory data; downloaded files remain on the device.",
  name: "Participant's name",
  email: "Participant's email address",
  conflict: "The same email address appears with different names",
  recommended: "Recommended based on the allocation export",
  recoveryText: "Correct the named file, then use Replace on that source, or remove and load it again. Review and regenerate the outputs. Closing or resetting clears memory, so sources must be reloaded.",
  sourceRejectedIssue: "The source could not be read. Correct the file, then remove or replace the source; an older state is never reused silently.",
});
text.ro.sourcePendingIssue="Verificarea locală este în curs; exportul de producție rămâne blocat.";
text.en.sourcePendingIssue="Local checking is in progress; production export remains blocked.";
...
function issueMessage(issue){if(issue.code==="source-pending")return tr("sourcePendingIssue");const ro={"project-required":"Completează numele proiectului.","files-required":"Încarcă cel puțin un fișier de respondenți.","source-rejected":tr("sourceRejectedIssue"),"participant-name-missing":"Completează numele participantului.","participant-email-invalid":"Completează un email valid pentru participant.","self-count":"Este necesar un singur rând de autoevaluare.","too-many":"Sunt mai mult de 15 respondenți.","evaluator-name-missing":"Lipsește numele unui respondent.","evaluator-email-invalid":"Email de respondent invalid.","unknown-role":"Rol necunoscut; corectează fișierul sursă.","language-invalid":"Limba trebuie să aibă două litere.","duplicate-allocation":"Același respondent apare de două ori pentru participant.","manager-required":"Lipsește un Manager distinct de participant.","name-conflict":"Confirmă numele canonic pentru email.","allocation-invalid":"Rând invalid în exportul de alocări.","email-multiple-identifiers":"Un email are mai mulți identificatori.","identifier-multiple-emails":"Un identificator aparține mai multor emailuri."};const en={"project-required":"Enter the project name.","files-required":"Load at least one respondent file.","source-rejected":tr("sourceRejectedIssue"),"participant-name-missing":"Enter the participant name.","participant-email-invalid":"Enter a valid participant email.","self-count":"Exactly one self-evaluation row is required.","too-many":"There are more than 15 respondents.","evaluator-name-missing":"A respondent name is missing.","evaluator-email-invalid":"A respondent email is invalid.","unknown-role":"Unknown role; correct the source workbook.","language-invalid":"Language must use two letters.","duplicate-allocation":"The same respondent appears twice for this participant.","manager-required":"A Manager distinct from the participant is missing.","name-conflict":"Confirm the canonical name for this email.","allocation-invalid":"Invalid row in the allocation export.","email-multiple-identifiers":"One email has multiple identifiers.","identifier-multiple-emails":"One identifier belongs to multiple emails."};return(lang==="ro"?ro:en)[issue.code]||issue.code}
...
async function attemptSource(file,kind,replaceId=null){q("#global-message").hidden=true;const pending=beginSourceAttempt(file,kind,replaceId);let stage="read";let data;let sha256="unavailable";try{data=await readFile(file);stage="digest";sha256=await sha256Hex(data);stage="parse";const parsed=kind==="allocation"?parseAllocationWorkbook(window.XLSX,data,file.name):parseParticipantWorkbook(window.XLSX,data,file.name);settleSourceAttempt(pending,{kind,name:file.name,bytes:file.size,sha256,adapter:kind==="allocation"?"allocation":parsed.type,parsed})}catch(error){const failureStage=stage;const errorCode=failureStage==="read"?"source-read-failed":failureStage==="digest"?"source-digest-failed":"source-parse-failed";settleSourceAttempt(pending,{kind,name:file.name,bytes:file.size,sha256,adapter:"rejected",attemptState:"rejected",failureStage,errorCode,errorDetail:error?.message||`${kind}-error`});q("#global-message").textContent=`${file.name}: ${tr(kind==="allocation"?"allocationError":"unsupported")}. ${tr("sourceRejectedIssue")}`;q("#global-message").hidden=false}nameChoices={};render()}

q("#participant-files").addEventListener("change",async(event)=>{for(const file of event.target.files)await attemptSource(file,"participant");event.target.value=""});
q("#allocation-file").addEventListener("change",async(event)=>{const file=event.target.files[0];if(file)await attemptSource(file,"allocation");event.target.value=""});
q("#project-name").addEventListener("input",render);
q("#reset").addEventListener("click",()=>{if(!window.confirm(tr("clearConfirm")))return;sourceLedger=[];nameChoices={};q("#project-name").value="";q("#global-message").hidden=true;render()});
